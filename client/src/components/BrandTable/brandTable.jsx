...
import { getBrands, removeBrand } from "../../redux/actions/brand_actions";
...
const BrandTable = () => {
    ...
    const brandArray = useSelector(
        (state) => state.brandReducer.allBrands?.brands
    );

    const isLoading = useSelector(
        (state) => state.brandReducer.isLoading
    )
    //TODO : ADD PAGINATION 
        ...
        <div className="tracking-wide font-bold">
            <div className="mt-4 pt-20 mb-4  flex justify-center">
                ...
                <div className="relative mr-6 my-2 ml-2 -mt-0.5">
                    ...
                    <button className="bg-gray" title="Search Brand">🔍</button>
            ...
            </div >
            {
                ...
                <table className="border-collapse w-full">
                    <thead>
                        <tr>
                            <th className="p-3 font-bold uppercase bg-gray-200 text-gray-600 border border-gray-300 hidden lg:table-cell">Brand ID</th>
                            <th className="p-3 font-bold uppercase bg-gray-200 text-gray-600 border border-gray-300 hidden lg:table-cell">Brand Name</th>
                            <th className="p-3 font-bold uppercase bg-gray-200 text-gray-600 border border-gray-300 hidden lg:table-cell">Brand Description</th>
                            <th className="p-3 font-bold uppercase bg-gray-200 text-gray-600 border border-gray-300 hidden lg:table-cell">Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        {brandArray && brandArray.length > 0 ? brandArray.map((c, id) => {
                            return <tr key={id} className="bg-white lg:hover:bg-gray-100 flex lg:table-row flex-row lg:flex-row flex-wrap lg:flex-no-wrap mb-10 lg:mb-0">
                                ...
                                <div className="flex gap-5 justify-center" >
                                                <button type="submit" title="Edit Category" className="flex text-white bg-indigo-500 border-0  mt-4 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded" type="submit"><Link to={'/editBrand/' + c._id}><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="M18.363 8.464l1.433 1.431-12.67 12.669-7.125 1.436 1.439-7.127 12.665-12.668 1.431 1.431-12.255 12.224-.726 3.584 3.584-.723 12.224-12.257zm-.056-8.464l-2.815 2.817 5.691 5.692 2.817-2.821-5.693-5.688zm-12.318 18.718l11.313-11.316-.705-.707-11.313 11.314.705.709z"/></svg></Link></button>
